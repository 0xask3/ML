...
def load_class_values(filename: str) -> np.array:
    ...
    """
    data = pd.read_csv(filename, sep="\t")
    # Filter the overall quality ratings (0s for score 1, 1s for scores 2 or 3)
    class_values = data["overall quality"].apply(lambda x: 0 if x == 1 else 1).tolist()
...
def misclassification_rate(cs: np.array, ys: np.array) -> float:
    ...
    if len(cs) != len(ys):
        raise ValueError(
            "The lengths of gold_labels and predicted_labels should be the same."
        )
    # Calculate the misclassification rate
    misclass_rate = np.mean(cs != ys)
...
def logistic_function(w: np.array, x: np.array) -> float:
    ...
    divisions by zero
    """
    # Calculate the dot product of input x and weights w
    z = np.dot(np.transpose(w), x)
    # Calculate the sigmoid/logistic function output
    sigmoid_output = 1 / (1 + np.exp(np.clip(-1 * z, -30, 30)))
...
def logistic_loss(w: np.array, x: np.array, c: int) -> float:
    ...
    """
    p = logistic_function(w, x)
    return -np.mean(c * np.log(p) + (1 - c) * np.log(1 - p))


def train_logistic_regression_with_bgd(
    xs: np.array,
    cs: np.array,
    eta: float = 1e-8,
    iterations: int = 1000,
    ...
    """
    weights = initialize_random_weights(len(xs.T))
    delta = 0
    split_idx = (np.rint(validation_fraction * len(xs))).astype(int)
    X_train, X_val = xs[:split_idx], xs[split_idx:]
    Y_train, Y_val = cs[:split_idx], cs[split_idx:]
    ...
    for i in range(iterations):
        ...
        for feature in X_train:
            loss = logistic_loss(weights, feature, cs[counter])
            for i in range(len(feature)):
                tempDelta += (cs[counter] - logistic_function(weights, feature)) * feature[i]
            delta += eta * tempDelta
            losses.append(loss)
            counter += 1
        weights += delta
        ...
        for data in X_train:
            training_predicted.append(logistic_prediction(weights, data))
        for data in X_val:
            test_predicted.append(logistic_prediction(weights, data))
        train_misclass_rate.append(misclassification_rate(training_predicted, Y_train))
        test_misclass_rate.append(misclassification_rate(test_predicted, Y_val))

        if len(lossList) > 0:
            lossList.append(np.mean(losses))

    return weights, lossList, train_misclass_rate, test_misclass_rate


def plot_loss_and_misclassification_rates(
    losss: List[float],
    ...
    for each iteration.
    """
    plt.plot(losss, label="losss")
    plt.plot(validation_misclassification_rates, label="test misclassification")
    plt.plot(train_misclassification_rates, label="train misclassification")
    plt.show()
...
if __name__ == "__main__":
    ...
    print("(b)")
    ys = np.random.choice([0, 1], size=len(cs), p=(class_counts / (len(cs))))
    mcl_rate = misclassification_rate(cs, ys)
    print(f"Misclassification rate of random classifier: {mcl_rate * 100}")
    print("(c)")
    test_c_result = pytest.main(
        ["-k", "test_logistic_function", "--tb=short", __file__]
    )
    if test_c_result != 0:
    ...
    ) = train_logistic_regression_with_bgd(xs, cs, validation_fraction=0.2)
    print("(e)")
    plot_loss_and_misclassification_rates(
        losss, train_misclassification_rates, validation_misclassification_rates
    )

    print("(f)")
    output = logistic_prediction(w,xs_test.T)
    opToWrite = pd.DataFrame(data=output)
    opToWrite.to_csv(test_predictions_file_name, sep="\t", index=False, header=False)
